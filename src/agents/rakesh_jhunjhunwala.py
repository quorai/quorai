...
def rakesh_jhunjhunwala_agent(state: AgentState, agent_id: str = "rakesh_jhunjhunwala_agent"):
    ...
    for ticker in tickers:
        ...
        financial_line_items = bundle.line_items
        ...
        if margin_of_safety is not None:
            _confidence = min(max(abs(margin_of_safety) * 150, 20), 95)  # 20-95% range
        else:
            _confidence = min(max((total_score / max_score) * 100, 10), 80)  # Based on score
...
def analyze_rakesh_jhunjhunwala_style(
    ...
    details = f"Profitability: {profitability['details']}\nGrowth: {growth['details']}\nBalance Sheet: {balance_sheet['details']}\nCash Flow: {cash_flow['details']}\nManagement Actions: {management['details']}"
