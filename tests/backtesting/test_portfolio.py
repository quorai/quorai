...
def test_apply_long_buy_fractional_shares() -> None:
    p = Portfolio(tickers=["AAPL"], initial_cash=200.0, margin_requirement=0.5)
    # Buy 1.5 shares @ $100 → cost $150, cash $50
    executed = p.apply_long_buy("AAPL", quantity=1.5, price=100.0)
    assert executed == pytest.approx(1.5)
    snap = p.get_snapshot()
    assert snap["positions"]["AAPL"]["long"] == pytest.approx(1.5)
    assert snap["cash"] == pytest.approx(50.0)


def test_apply_long_sell_fractional_shares() -> None:
    p = Portfolio(tickers=["AAPL"], initial_cash=500.0, margin_requirement=0.5)
    p.apply_long_buy("AAPL", quantity=1.5, price=100.0)
    # Sell 0.5 shares @ $100 → position 1.0 remains
    executed = p.apply_long_sell("AAPL", quantity=0.5, price=100.0)
    assert executed == pytest.approx(0.5)
    snap = p.get_snapshot()
    assert snap["positions"]["AAPL"]["long"] == pytest.approx(1.0)


def test_apply_short_open_fractional_shares() -> None:
    p = Portfolio(tickers=["AAPL"], initial_cash=500.0, margin_requirement=0.5)
    # Short 2.5 shares @ $100 → proceeds 250, margin 125
    executed = p.apply_short_open("AAPL", quantity=2.5, price=100.0)
    assert executed == pytest.approx(2.5)
    snap = p.get_snapshot()
    assert snap["positions"]["AAPL"]["short"] == pytest.approx(2.5)


def test_apply_short_cover_fractional_shares() -> None:
    p = Portfolio(tickers=["AAPL"], initial_cash=1_000.0, margin_requirement=0.5)
    p.apply_short_open("AAPL", quantity=2.5, price=100.0)
    # Cover 1.25 shares → short position should be 1.25
    executed = p.apply_short_cover("AAPL", quantity=1.25, price=100.0)
    assert executed == pytest.approx(1.25)
    snap = p.get_snapshot()
